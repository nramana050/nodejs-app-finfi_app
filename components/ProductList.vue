  ...
  div
    div.ps-5
      div.ps-2(v-for="item in this.productList" :key="item.id" @click="navToStartPlan(); selectProduct(item)")
        ...
        div.ps-4
          div.text-xs.ps-4A.font-bold {{item.product_name}}
          div.font-bold.text-sm.ps-4B Save for {{item.product_name}} products
          div.text-sm Bonus reward Upto {{item.merchant_discount}}%
