    ...
    div
      ...
      div.ps-2
        div.ps-4.flex.flex-row.justify-between
          div.relative.pt-1
            label.form-label(for='customRange1') Your goal amount
        ...
        div.ps-4.flex.flex-row.justify-between
          div.relative.pt-1
            label.form-label(for='customRange1') Duration
            span.ps-10 {{slidervalue2}} Months
            input#customRange1.form-range.w-full.h-6.p-0.bg-transparent(type='range' class='focus:outline-none focus:ring-0 focus:shadow-none' min="0" max="12" v-model="slidervalue2")
...
export default {
  ...
  mounted() {
    ...
    setInterval(function () {
      ...
      _this.getAmount = parseFloat(_this.slidervalue1 / 12).toFixed(0)
    }, 1000)
  },
  methods: {
    navToProductScreen() {
    ...
    createOrder() {
      ...
      this.$axios.$post(`/snbl/order`, payload).then((result) => {
