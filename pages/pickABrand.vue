...
div.ps-3.shop-container
    div.flex-0 
      PageHeader.font-bold(title="Pick A Brand")
      div.main-container
        div.px-4.mb-2(v-if="allProductList?.length>8")
          input(type='text' id="search-product" placeholder="Search" @input="searchProduct")  
        div.container.corp-exp.products.p-2
          div.latest-claim(v-if="filteredProductList?.length")
                div.slide.custom-pro-slide(v-for="product in filteredProductList" @click="addToCart({ product: product?.product})") 
                  div.slide-header
...
export default {
  ...
  mounted() {
    this.getAllCategoryAllProducts()
  },
  methods: {
    searchProduct(e) {
      ...
      if (keyword) {
        ...
        this.filteredProductList = this.allProductList?.filter((OBJ) => {
