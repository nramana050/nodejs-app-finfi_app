      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            this.marker_list = [ ]
            this.marker_list.push(new window.google.maps.Marker({
              position: pos,
              map : this.map,
              title: "Your Location",
            }));
            ...
            axios.post('http://localhost:8003/locationdata', locData)
             .then(response => {
    ...
    updateCurrentLocation() {
        ...
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            console.log("updateCurrentLocation: this.locArr.toString() = ", this.locArr.toString())

            if(this.locArr.length> 0 && 
            this.calcCrow(pos.lat,pos.lng, 
...
getTrailHandler(){
  ...
  for (let count = 0; count < self.locArr.length; count++) {
    const marker = new window.google.maps.Marker({
      position: new window.google.maps.LatLng(self.locArr[count].lat, 
